...
if [ "$#" -lt 2 ] || [ -z "${1:-}" ] || [ -z "${2:-}" ]; then
  echo "Usage: $0 <HOST_IP> <BOOTSOURCE> [--expect-firmware]" >&2
  exit 1
fi
...
KERNEL_SHA=$(docker exec kind-control-plane \
  sha256sum "/opt/isoboot/files/${BOOTSOURCE}/linux" | awk '{print $1}')

echo "  kernel sha256: ${KERNEL_SHA}"

# Check for firmware (determines directory layout)
HAS_FIRMWARE=false
if docker exec kind-control-plane test -d "/opt/isoboot/files/${BOOTSOURCE}/no-firmware" 2>/dev/null; then
  HAS_FIRMWARE=true
  # With firmware: initrd in subdirectories
  INITRD_NO_FW_SHA=$(docker exec kind-control-plane \
    sha256sum "/opt/isoboot/files/${BOOTSOURCE}/no-firmware/initrd.gz" | awk '{print $1}')
  INITRD_WITH_FW_SHA=$(docker exec kind-control-plane \
    sha256sum "/opt/isoboot/files/${BOOTSOURCE}/with-firmware/initrd.gz" | awk '{print $1}')
  INITRD_NO_FW_SIZE=$(docker exec kind-control-plane \
    stat -c%s "/opt/isoboot/files/${BOOTSOURCE}/no-firmware/initrd.gz")
  INITRD_WITH_FW_SIZE=$(docker exec kind-control-plane \
    stat -c%s "/opt/isoboot/files/${BOOTSOURCE}/with-firmware/initrd.gz")
  ...
  echo "FAIL: firmware expected but no-firmware/ directory not found for ${BOOTSOURCE}"
  exit 1
else
  # No firmware: initrd at top level
  INITRD_SHA=$(docker exec kind-control-plane \
    sha256sum "/opt/isoboot/files/${BOOTSOURCE}/initrd.gz" | awk '{print $1}')
  echo "  initrd sha256: ${INITRD_SHA}"
fi
echo ""

TMPDIR="/tmp/static-content-${BOOTSOURCE}"
mkdir -p "$TMPDIR"
...
test_invalid_file_404() {
  ...
  code=$(curl -s -o /dev/null -w '%{http_code}' \
    "${BASE_URL}/static/${BOOTSOURCE}/nonexistent")
  [ "$code" = "404" ]
}

test_kernel() {
  curl -f -s -o "${TMPDIR}/kernel" \
    "${BASE_URL}/static/${BOOTSOURCE}/linux"
  local sha
...
test_initrd_no_firmware() {
  curl -f -s -o "${TMPDIR}/initrd-no-fw" \
    "${BASE_URL}/static/${BOOTSOURCE}/no-firmware/initrd.gz"
  local sha
...
test_initrd_with_firmware() {
  curl -f -s -o "${TMPDIR}/initrd-with-fw" \
    "${BASE_URL}/static/${BOOTSOURCE}/with-firmware/initrd.gz"
  local sha
...
test_initrd_flat() {
  curl -f -s -o "${TMPDIR}/initrd" \
    "${BASE_URL}/static/${BOOTSOURCE}/initrd.gz"
  local sha
...
echo "static-content/${BOOTSOURCE}: ${PASSED}/${TESTS} passed"

# Write machine-readable results for the wrapper
echo "${PASSED} ${TESTS}" > "/tmp/static-content-${BOOTSOURCE}.results"
